...
synced_uuid | string | 同步日历的唯一识别码，来源于第三方日历本身
name  | string | 日历名称
...
**URL:** `GET` calendar/device/<device_id\>/synced/list
...
**URL:** `get` calendar/device/<device_id\>/synced/<synced_calendar_id\>/detail
...
**URL:** `POST` calendar/device/<device_id\>/synced/by_link

**输入:**

字段 | 类型 | 必填 | 说明
---|---|---|---
calendar_type  | int | 是 | 日历类型，google不是通过这个接口同步的 <br>2:outlook(/hotmail/live), 3:iCloud <br>4:Cozi, 5:Yahoo, 6:calendar url, 7:us holidays
link | string | 是 | 如果类型是7:us holidays, 填入空字符串""
...
**URL:** `POST` calendar/device/<device_id\>/synced/google

**输入:**

字段 | 类型 | 必填 | 说明
---|---|---|---
synced_style | int | 是 | 同步风格，1:单向同步，2:双向同步
...
**URL:** `POST` calendar/device/<device_id\>/synced/<synced_id\>/delete
...
**URL:** `GET` calendar/device/<device_id\>/category/list
...
**URL:** `GET` calendar/device/<device_id\>/category/<category_id\>/detail
...
**URL:** `POST` calendar/device/<device_id\>/category/add
...
**URL:** `POST` calendar/device/<device_id\>/category/<category_id\>/update
...
**删除分类，同时更换Event到另一个分类**

**URL:** `POST` calendar/device/<device_id\>/category/<category_id\>/delete

**输入:**

字段 | 类型 | 必填 | 说明
---|---|---|---
another_id  | int | 是 | 另一个分类id
...
**URL:** `GET` calendar/device/<device_id\>/event/list?start_at=&end_at=
...
**URL:** `GET` calendar/device/<device_id\>/event/<event_id\>/detail
...
**URL:** `POST` calendar/device/<device_id\>/event/add
...
**URL:** `POST` calendar/device/<device_id\>/event/<event_id\>/update
...
**URL:** `POST` calendar/device/<device_id\>/event/<event_id\>/delete
